...
export default function BottomNav() {
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] max-w-md glass-strong rounded-full px-6 py-4 flex justify-between items-center z-50 border border-white/20 lg:hidden">
      {navItems.map((item, i) => (
        <div key={item.href} className="flex items-center">
          {i === 2 && (
            <Link
              href="/contact"
              className="bg-neon p-2.5 rounded-full -mt-10 border-4 border-dark-bg shadow-lg shadow-neon/20 transform hover:scale-110 active:rotate-45 transition-all mr-6"
            >
              <svg className="h-6 w-6 text-dark-bg" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <Link
            ...
            className={`transform active:scale-90 transition-transform ${
              pathname === item.href ? 'text-neon' : 'text-gray-500'
            }`}
          >
            {item.icon}
          </Link>
