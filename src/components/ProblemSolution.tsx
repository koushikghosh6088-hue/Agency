            ...
            >
              {painPoints.map((point, index) => (
                <ScrollStackItem key={point.id}>
                  <div className="relative h-full w-full rounded-[2rem] p-6 md:p-10 overflow-hidden">
                    {/* Technical Visuals */}
                    ...
                    <div className="flex items-center gap-5 mb-8 relative z-10">
                       <div className={`w-14 h-14 rounded-2xl flex items-center justify-center bg-red-500/10 border border-red-500/30 shadow-[0_0_20px_rgba(239,68,68,0.1)]`}>
                          {(() => {
                             const Icon = point.icon;
                             return <Icon className={`w-7 h-7 text-red-500`} />;
                          })()}
                       </div>
                       <div>
                          <div className="flex items-center gap-2 mb-1">
                            <span className="font-mono text-[10px] uppercase tracking-[0.4em] text-red-500/50 font-bold">{point.id}</span>
                            <div className="h-[1px] w-8 bg-red-500/20" />
                          </div>
                          <h3 className="text-2xl md:text-3xl font-heading font-black text-white uppercase tracking-tighter leading-none">
                             {point.problem}
                          </h3>
                       </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-5 relative z-10">
                       <div className="p-6 rounded-[1.5rem] bg-white/[0.02] border border-white/5 backdrop-blur-sm">
                          <div className="flex items-center gap-2 mb-3">
                             <div className="w-1.5 h-1.5 rounded-full bg-red-600" />
                             <span className="text-[10px] font-mono text-red-500/80 uppercase tracking-[0.2em] font-bold">Diagnostic_Report</span>
                          </div>
                       ...
                       <div className={`p-6 rounded-[1.5rem] bg-red-500/[0.08] border border-red-500/30 shadow-[0_0_30px_rgba(239,68,68,0.05)]`}>
                          <span className={`text-[10px] font-mono text-red-400 uppercase tracking-[0.3em] mb-4 block font-black underline decoration-red-500/30 underline-offset-8`}>OPTIMIZED_SOLUTION</span>
                          <p className="text-lg md:text-2xl text-white font-black leading-tight tracking-tight uppercase">
                             {point.solution}
                          </p>
                       </div>
                    </div>
