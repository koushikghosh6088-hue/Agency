            ...
            <div className="max-w-[1400px] mx-auto px-6 relative z-10">
              <div className={`grid grid-cols-1 lg:grid-cols-12 gap-16 items-center ${i % 2 !== 0 ? 'lg:flex-row-reverse' : ''}`}>
                ...
                >
                  ...
                  <h2 className={`text-4xl md:text-5xl lg:text-6xl font-heading font-bold mb-6 ${service.accent ? 'text-lime-400' : 'text-white'}`}>
                    {service.title}
                  </h2>
                  ...
                  <div className="space-y-4 mb-10">
                    {service.benefits.map((benefit) => (
                      <div key={benefit} className="flex items-center gap-4">
                        <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 ${service.accent ? 'bg-lime-400' : 'bg-white/10'}`}>
                          <CheckCircle2 className={`w-3 h-3 ${service.accent ? 'text-black' : 'text-lime-400'}`} />
                        </div>
                  ...
                  <Link href="/contact" className={`group relative overflow-hidden px-8 py-4 rounded-full text-sm font-bold transition-all duration-300 inline-flex items-center gap-2 ${
                    service.accent ? 'bg-lime-400 text-black shadow-[0_0_30px_rgba(204,255,0,0.15)] hover:scale-105' : 'bg-white/5 border border-white/10 text-white hover:bg-white/10 hover:border-lime-400/40'
                  }`}>
                ...
                >
                  <div className={`glass-panel rounded-[2.5rem] p-8 md:p-12 relative overflow-hidden group min-h-[500px] flex flex-col justify-end ${
                    service.accent ? 'border-lime-400/20 hover:border-lime-400/50' : ''
                  }`}>
                    
                    {/* Background Gradients */}
                    <div className={`absolute inset-0 opacity-20 transition-opacity duration-500 group-hover:opacity-40 ${
                      service.accent ? 'bg-gradient-to-br from-lime-400 to-transparent' : 'bg-gradient-to-br from-white to-transparent'
                    }`} />
                    ...
                    <div className="relative z-20 glass px-6 py-6 rounded-2xl w-full max-w-sm ml-auto backdrop-blur-xl border-white/10 mt-auto">
                      <div className="flex items-center justify-between mb-4 border-b border-white/5 pb-4">
                        ...
                        <service.icon className={`w-4 h-4 ${service.accent ? 'text-lime-400' : 'text-white'}`} />
                      </div>
