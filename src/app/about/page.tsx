      ...
      <section className="relative pt-32 pb-20 overflow-hidden min-h-[60vh] flex flex-col justify-center">
        ...
        <div className="max-w-[1550px] mx-auto px-6 relative z-10 w-full">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: "easeOut" }}>
            <span className="font-mono text-[10px] tracking-widest uppercase block mb-6 text-lime-400">
              <span className="w-1.5 h-1.5 bg-lime-400 rounded-full animate-pulse inline-block mr-2" />
              Agency Profile
            </span>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {stats.map((stat, i) => (
              <AnimatedSection key={stat.label} delay={i * 0.1}>
                <div className={`glass-panel border-white/5 rounded-[2rem] p-8 text-center group transition-all duration-500 hover:border-lime-400/30 hover:bg-white/5`}>
                  <div className="w-12 h-12 rounded-xl bg-white/5 flex items-center justify-center mx-auto mb-6 group-hover:bg-lime-400/10 transition-colors">
                    <stat.icon className="w-6 h-6 text-white group-hover:text-lime-400 transition-colors" />
                  </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
            ...
            <AnimatedSection className="lg:col-span-5 sticky top-32">
              ...
              <h2 className="text-4xl md:text-5xl lg:text-6xl font-heading font-bold mb-8 text-white">
                From Startup to <span className="text-lime-400">Global Agency</span>
              </h2>
            ...
            <AnimatedSection delay={0.2} direction="right" className="lg:col-span-6 lg:col-start-7">
              <div className="glass-panel border-lime-400/20 rounded-[3rem] p-8 md:p-12">
                <div className="space-y-0 relative before:absolute before:inset-y-0 before:left-[17px] md:before:left-[21px] before:w-px before:bg-gradient-to-b before:from-lime-400 before:via-white/10 before:to-transparent">
                  {[
                    ...
                    >
                      <div className="absolute left-0 top-1/2 -translate-y-1/2 w-9 h-9 md:w-11 md:h-11 rounded-full bg-black border border-white/20 flex items-center justify-center group-hover:border-lime-400 transition-colors">
                        <div className="w-2 h-2 rounded-full bg-white/40 group-hover:bg-lime-400 transition-colors" />
                      </div>
                      <div className="text-2xl font-bold font-heading text-white group-hover:text-lime-400 transition-colors">{item.year}</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {values.map((value, i) => (
              <AnimatedSection key={value.title} delay={i * 0.1}>
                <div className="glass-panel border-white/10 rounded-[2rem] p-8 h-full group hover:bg-white/5 transition-all duration-500 hover:border-lime-400/30">
                  <div className="w-14 h-14 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center mb-6 group-hover:scale-110 group-hover:bg-lime-400/10 transition-all">
                    <value.icon className="w-6 h-6 text-white/50 group-hover:text-lime-400 transition-colors" />
                  </div>
                  <h3 className="text-xl font-heading font-bold mb-3 text-white group-hover:text-lime-400 transition-colors">{value.title}</h3>
              ...
              <AnimatedSection key={member.name} delay={i * 0.1}>
                <div className="group text-center cursor-default">
                  <div className={`w-full aspect-square rounded-[2rem] border border-white/10 ${
                    i % 2 === 0 ? 'bg-lime-400/5 hover:border-lime-400/30' : 'bg-white/5 hover:border-white/30'
                  } mb-6 flex items-center justify-center group-hover:scale-105 group-hover:shadow-[0_0_30px_rgba(204,255,0,0.1)] transition-all duration-500 overflow-hidden relative`}>
                    
                    {/* Placeholder for actual photo, using geometric abstraction for now */}
                    <div className="absolute inset-x-0 bottom-0 h-1/2 bg-gradient-to-t from-black/50 to-transparent" />
                    <Users className={`w-10 h-10 ${i % 2 === 0 ? 'text-lime-400/50' : 'text-white/30'} group-hover:scale-110 transition-transform`} />
                    
                    {/* Scanline effect */}
                    <div className="absolute inset-0 bg-[linear-gradient(transparent_50%,rgba(0,0,0,0.1)_50%)] bg-[length:100%_4px] opacity-20" />
                  </div>
                  <h4 className="text-sm font-bold font-heading text-white mb-1 group-hover:text-lime-400 transition-colors text-balance">{member.name}</h4>
        ...
        <div className="max-w-[1550px] mx-auto px-6 text-center relative z-10">
          <AnimatedSection>
            <h2 className="text-[4rem] md:text-[5rem] font-heading font-black tracking-tighter mb-6">
              ENGAGE <span className="text-lime-400 italic">SYSTEMS</span>
            </h2>
