            ...
            {categories.map((cat) => (
              <button
                ...
                className={`px-6 py-2.5 rounded-full text-xs font-mono uppercase tracking-widest transition-all duration-300 ${
                  activeFilter === cat
                    ? 'bg-lime-400 text-black shadow-[0_0_20px_rgba(204,255,0,0.2)] font-bold'
          ...
          <motion.div layout className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6 auto-rows-[300px]">
            <AnimatePresence mode="popLayout">
              {filtered.map((project, i) => (
                ...
                >
                  <div className="group glass-panel border-white/5 rounded-[2.5rem] overflow-hidden cursor-pointer h-full relative flex flex-col justify-end hover:border-lime-400/30 transition-colors duration-500">
                    
                    {/* Dark gradient overlay */}
                    <div className={`absolute inset-0 ${
                      i % 3 === 0 ? 'bg-gradient-to-t from-lime-400/10 via-black/80 to-black/20' : 'bg-gradient-to-t from-white/10 via-black/80 to-black/20'
                    ...
                    <div className="relative z-10 p-8 h-full flex flex-col justify-end">
                      ...
                      <h3 className={`font-heading font-bold mb-3 text-white group-hover:text-lime-400 transition-colors ${
                      ...
                      <div className="mt-8 flex items-center gap-2 text-lime-400 text-sm font-bold opacity-0 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 transition-all duration-300">
